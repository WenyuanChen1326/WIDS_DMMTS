...
df = df %>% select(features, treatment_pd, allocated_set)
```
...
# Impute using all the available data (train/test set)
df_imputed = mice(df %>% select(-treatment_pd, -allocated_set), method = "rf", seed = 123) %>% complete()

# Add back outcome variable and allocated set
df_imputed = df_imputed %>% bind_cols(df %>% select(treatment_pd, allocated_set))
```
